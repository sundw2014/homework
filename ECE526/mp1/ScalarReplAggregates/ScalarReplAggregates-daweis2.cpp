...
bool SROA::expandAggreateAlloca(AllocaInst *AI,
  ...
  if (!dyn_cast<StructType>(type) && !dyn_cast<ArrayType>(type))
    return false;
  ...
  for (auto I = userList.begin(), E = userList.end(); I != E; I++) {
    ...
    if (GetElementPtrInst *GEPI = dyn_cast<GetElementPtrInst>(U)) {
      if (GEPI->getNumOperands() == 3) {
        ...
        // errs() << "\n";
        // errs() <<
        // "dyn_cast<PointerType>(GEPI->getPointerOperandType())->getElementType()->print():
        // ";
        // dyn_cast<PointerType>(GEPI->getPointerOperandType())->getElementType()->print(errs());
        // errs() << "\n";
        // auto STy =
        // cast<StructType>(dyn_cast<PointerType>(GEPI->getPointerOperandType())->getElementType());
